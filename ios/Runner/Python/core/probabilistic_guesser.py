...
def find_5050_situations(probability_map):
    """
    Given a probability map (cell: probability), return a list of cells
    that are true 50/50 situations (i.e., probability == 0.5).
    """
    return [cell for cell, prob in probability_map.items() if abs(prob - 0.5) < 1e-6]


def find_5050_situations_from_dict(probability_map):
    """
    Given a probability map (cell: probability) as a Python dict, return a list of cells
    that are true 50/50 situations (i.e., probability == 0.5).
    This is the preferred API for integration with Swift/PythonKit.
    Args:
        probability_map (dict): Keys are (row, col) tuples or strings, values are floats.
    Returns:
        list: List of (row, col) tuples that are 50/50 situations.
    """
    def parse_key(key):
        if isinstance(key, str) and key.startswith("("):
            return tuple(map(int, key.strip("() ").split(",")))
        return key
    parsed_map = {parse_key(k): v for k, v in probability_map.items()}
    return find_5050_situations(parsed_map)


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Find 50/50 situations from a probability map.")
    parser.add_argument("--probability_map", type=str, required=True, help="Path to JSON file with probability map.")
    args = parser.parse_args()

    with open(args.probability_map, "r") as f:
        probability_map = json.load(f)
    # Convert keys from string to tuple if needed
    def parse_key(key):
        if isinstance(key, str) and key.startswith("("):
            return tuple(map(int, key.strip("() ").split(",")))
        return key
    parsed_map = {parse_key(k): v for k, v in probability_map.items()}
    result = find_5050_situations(parsed_map)
